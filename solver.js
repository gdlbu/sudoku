...
function computeCandidatesMap(board) {
  return board.map((row, rowIndex) =>
    row.map((value, colIndex) => {
      ...
      return getCandidates(board, rowIndex, colIndex);
    }),
  );
}
...
async function applyDeterministic(board) {
  ...
  while (true) {
    const candidatesMap = computeCandidatesMap(board);
    let progress = false;
...
function selectNextCell(board, candidatesMap) {
  ...
  for (let row = 0; row < GRID_SIZE; row += 1) {
    for (let col = 0; col < GRID_SIZE; col += 1) {
      ...
      const candidates = candidatesMap ? candidatesMap[row][col] : getCandidates(board, row, col);
...
function applyTheme(mode) {
  ...
  if (mode === 'dark') {
    ...
    if (themeToggleButton) {
      ...
      themeToggleButton.setAttribute('aria-label', '当前暗色模式，点击切换为浅色模式');
    }
  } else if (mode === 'light') {
    document.body.setAttribute('data-theme', 'light');
    if (themeToggleButton) {
      themeToggleButton.textContent = '☀️';
      themeToggleButton.setAttribute('aria-label', '当前浅色模式，点击切换为系统默认');
    }
  } else {
    document.body.removeAttribute('data-theme');
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    if (themeToggleButton) {
      themeToggleButton.textContent = prefersDark ? '🌙' : '☀️';
      themeToggleButton.setAttribute('aria-label', `跟随系统主题（当前${prefersDark ? '暗色' : '浅色'}），点击切换为暗色模式`);
    }
  }
